...
import { FaCalendarAlt, FaUserCircle, FaQuoteLeft, FaThumbsUp } from "react-icons/fa";
...
export default function Blog() {
    ...
    const fetchInitialLikes = async () => {
        ...
        try {
            const response = await axios.post(`/api/initialLikes/${id}`, { userId: user._id });
  ...
  <h2 className="text-2xl font-semibold text-gray-800 mb-4">Give Your Vote!</h2>
  <div className="flex justify-center items-center gap-6">
    <div className="flex items-center gap-2">
      <span className="text-4xl font-bold text-indigo-700">{blogLikes}</span>
  ...
  <p className="text-sm text-gray-500 mt-4">{hasLiked ? "You liked this post" : "Like this post?"}</p>
</div>
